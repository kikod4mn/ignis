...
use App\Entity\User;
use App\Form\User\Security\ChangePasswordType;
use App\Form\User\Security\ResetPasswordRequestType;
...
class ResetPasswordController extends AbstractController {
	...
	/**
	 * @Route("/forgot-password", name="forgot-password-request")
	 */
	public function request(Request $request, MailerInterface $mailer): Response {
		$form = $this->createForm(ResetPasswordRequestType::class);
		$form->handleRequest($request);
		if ($form->isSubmitted() && $form->isValid()) {
			return $this->processSendingPasswordResetEmail($form->get('_email')->getData(), $mailer);
		}
	...
	public function reset(Request $request, UserPasswordEncoderInterface $passwordEncoder, string $token = null): Response {
		...
		try {
			...
			$this->addFlash(
				'reset_password_error', sprintf('There was a problem validating your reset request - %s', $e->getReason())
			);
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			$encodedPassword = $passwordEncoder->encodePassword($user, $form->get('_plainPassword')->getData());
